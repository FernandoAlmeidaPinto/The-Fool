...
**Goal:** Replace the "Em breve" placeholder on `/leituras` with a paginated list of the user's past readings.

**Architecture:** Add a `listUserInterpretations` service function with pagination, then update the hub page to fetch and display results with server-side pagination via URL query params.

**Tech Stack:** Next.js 16 Server Components, Mongoose, Tailwind CSS 4

**Spec:** `docs/superpowers/specs/2026-04-05-reading-history-design.md`

---

### Task 1: Add listUserInterpretations to service

**Files:**
- Modify: `lib/readings/service.ts`

- [ ] **Step 1: Add the paginated list function**

Add to the end of `lib/readings/service.ts` (after `getCombinationById`):
...
git commit -m "feat: add listUserInterpretations with pagination"
```

---

### Task 2: Update hub page with reading history list
...
Replace the entire contents of `app/(dashboard)/leituras/page.tsx` with:
...
- `searchParams` is async in Next.js 16 — must `await searchParams`
- Profile and history fetched in parallel via `Promise.all`
- Deck data resolved by collecting distinct `deckId` values and fetching in parallel
- Cards resolved from deck subdocuments (same pattern as result page)
- Date formatted with `toLocaleDateString("pt-BR")`
- Pagination uses plain `<Link>` elements (not buttons) for server-side navigation
- Invalid page redirects to `/leituras` (no query param = page 1)
...
git commit -m "feat: add reading history list with pagination to leituras hub"
```
